...
SUFFIX=$(echo $RANDOM)
NETWORK="liquidlight-form-to-database-${SUFFIX}"
...
case ${TEST_SUITE} in
    cgl)
        if [ "${CGLCHECK_DRY_RUN}" -eq 1 ]; then
            ...
            COMMAND="php -dxdebug.mode=off .Build/bin/php-cs-fixer fix --config Build/php-cs-fixer/config.php --using-cache no"
        fi
